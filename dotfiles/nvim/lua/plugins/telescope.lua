...
function plugin.config()
  ...
  vim.keymap.set("n", "<Leader>fa", function()
    tb.find_files({
      find_command = { "@fd@/bin/fd", "--type=file", "--size=-1M", "--hidden", "--strip-cwd-prefix", "--no-ignore" },
    })
  end)
  vim.keymap.set("n", "<Leader>ff", function()
    tb.find_files({
      find_command = { "@fd@/bin/fd", "--type=file", "--size=-1M", "--hidden", "--strip-cwd-prefix", "--exclude=.git" },
    })
  end)
