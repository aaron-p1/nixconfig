(local {: nvim_tabpage_list_wins
        : nvim_win_get_buf
        : nvim_buf_get_option
        : nvim_list_tabpages
        : nvim_tabpage_is_valid} vim.api)

(local {: tabpagewinnr} vim.fn)

(lambda check-opt-in-tab [tabnr option]
  (let [wins (nvim_tabpage_list_wins tabnr)
        bufs (vim.tbl_map #(nvim_win_get_buf $1) wins)]
...
(fn config []
  ...
  (local llh (require :lualine.highlight))
  (local llu (require :lualine.utils.utils))
  (local devicons (require :nvim-web-devicons))
  (ll.setup {:options {:theme :onedark :globalstatus false}
       ...
       (fn [self]
         (let [hl (. self.highlights (if self.current :active :inactive))
               name (if self.ellipse "…"
                        (vim.trim (.. self.tabnr " " (self.label self) " "
           ...
           (.. (if self.first "" (self:separator_before))
               (llh.component_format_highlight hl) padded-name)))))
