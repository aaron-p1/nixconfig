    ...
    config =
      let
        ...
        normalizedProfiles = pipe profiles [
          (mapAttrs (
            _: profile:
            throwIfNot (all (n: hasAttr n functionConfigs) (
              attrNames profile
        ...
        unknownProfileFunctions = pipe profiles [
          (mapAttrs (_: profile: filterAttrs (name: _: !hasAttr name functionConfigs) profile))
