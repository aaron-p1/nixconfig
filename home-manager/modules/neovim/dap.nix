{ pkgs, lib, ... }:
{
  within.neovim.configDomains.dap =
    let
      ...
      inherit (lib) pipe mapAttrs' nameValuePair;

      dapConfigs = {
        php = import ./dap/php.nix { inherit pkgs; };
        rust = ./dap/rust.lua;
      };

      dapConfigRequires = pipe dapConfigs [
        attrNames
        (map (name: "require('dap-configs.${name}')"))
        (concatStringsSep "\n")
      ];
    in
    {
      ...
      config =
        ...
        ''
          ...
          vim.keymap.set("n", "<Leader><F5>", dap.run_last, { desc = "Rerun" })
        ...
        '';
      extraFiles.lua.dap-configs = mapAttrs' (
        name: source: nameValuePair "${name}.lua" source
      ) dapConfigs;
    };
}
