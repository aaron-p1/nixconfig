...
{ pkgs, lib, ... }:
{
  within.neovim.configDomains.snippets =
    let
      inherit (builtins)
        readDir
        mapAttrs
        ...
        pipe dir [
          ...
          (mapAttrsToList (
            ...
            in
            {
              ...
              options = if optionLine == null then { } else parseOptionLine optionLine;
      ...
      toLuaTable =
        attrs:
        let
          toLuaValue = v: if isString v then v else lib.throw "Unsupported value type";
          elems = (mapAttrsToList (k: v: "${k} = ${toLuaValue v}") attrs);
        in
        "{ ${concatStringsSep ", " elems} }";
      ...
      toSnippetLines = map (
        ...
        in
        ''
          ls.parser.parse_snippet(${toLuaTable snippetOptions}, [===[${s.snippet}]===])
        ''
      );
