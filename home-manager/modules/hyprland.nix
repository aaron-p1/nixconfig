...
let
  ...
  allMonitorList = map (monitor: "monitor = ${monitor}")
    (cfg.monitors ++ [ cfg.monitorFallback ]);

  monitorConfig = concatStringsSep "\n" allMonitorList;
in with lib; {
  options.within.hyprland = {
    monitors = mkOption {
      type = types.listOf types.str;
      default = [ ];
      description = "Monitor configs";
    };
    monitorFallback = mkOption {
      type = types.str;
      default = ",preffered,auto,1";
      description = "Monitor fallback config";
    };
  };
  ...
  config = mkIf osCfg.enable {
    wayland.windowManager.hyprland = {
      ...
      inherit (osCfg) package xwayland;

      enableNvidiaPatches = osCfg.nvidiaPatches;

      extraConfig = with pkgs;
        let launcher = "${rofi-wayland}/bin/rofi -show drun";
        in ''
