{ config, lib, ... }:
let
  inherit (lib) mkIf attrValues listToAttrs nameValuePair;
  ...
  dnscryptCloaks = listToAttrs (map
    (service: nameValuePair "${service.subdomain}.${rcfg.devDomain}" service.ip)
    services);

  reverseProxyHosts = listToAttrs (map (service:
    nameValuePair service.subdomain {
      dst = "http://${service.ip}";
      redirectRoot = service.redirectRoot;
    }) services);
in {
  options.within.networking.devService = let
    inherit (lib) mkEnableOption mkOption;
    inherit (lib.types) attrsOf submodule bool str nullOr;
  in {
